...
type State struct {
	...
	// patch acessos: scrollback. lines caem aqui em scrollUp, ANTES de
	// serem limpas — é o único lugar onde o conteúdo que sai por cima da
	// tela ainda existe. historia[0] é a mais antiga.
	historia    []line
	historiaMax int
}

// historiaPadrao é usado quando ninguém pediu WithScrollback: gera algo
// útil mesmo para quem só importa o pacote sem saber que a opção existe.
const historiaPadrao = 10000

func newState(w io.Writer) *State {
	return &State{
		w:             w,
		colorOverride: make(map[Color]Color),
		historiaMax:   historiaPadrao,
	}
}
...
// HistoryLen retorna quantas linhas de scrollback existem acima da tela
// atual (patch acessos).
func (t *State) HistoryLen() int {
	return len(t.historia)
}

// HistoryCell devolve a célula da linha de histórico idx (0 = mais
// antiga), coluna x. Fora dos limites — inclusive se a linha foi
// capturada com uma largura diferente da atual, por causa de um
// redimensionamento depois — devolve uma célula em branco, nunca pânico
// (patch acessos).
func (t *State) HistoryCell(x, idx int) Glyph {
	if idx < 0 || idx >= len(t.historia) {
		return Glyph{}
	}
	l := t.historia[idx]
	if x < 0 || x >= len(l) {
...
func (t *State) scrollUp(orig, n int) {
	n = clamp(n, 0, t.bottom-orig+1)
	// patch acessos: só entra pro histórico o que sai por CIMA da tela
	// principal — orig==0 é o topo, e a tela alternativa (vim/less/nano em
	// tela cheia) não deve poluir o scrollback do shell, do jeito que
	// nenhum terminal de verdade faz.
	if orig == 0 && t.mode&ModeAltScreen == 0 && t.historiaMax > 0 {
		for i := 0; i < n && i < len(t.lines); i++ {
			cp := make(line, len(t.lines[i]))
			copy(cp, t.lines[i])
			t.historia = append(t.historia, cp)
		}
		if excesso := len(t.historia) - t.historiaMax; excesso > 0 {
			t.historia = t.historia[excesso:]
		}
	}
