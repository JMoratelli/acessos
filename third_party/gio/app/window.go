...
func (w *Window) frame(frame *op.Ops, viewport image.Point) error {
	if runtime.GOOS == "js" {
		...
		// Original: limpa com BRANCO OPACO. Com decoração própria (CSD) e
		// cantos arredondados, os quatro cantos ficam fora do recorte do
		// app e sobravam como bicos brancos — bem visíveis no tema
		// escuro. Limpando com preto transparente, o que o app não pinta
		// simplesmente não existe, e o compositor compõe o que está
		// atrás.
