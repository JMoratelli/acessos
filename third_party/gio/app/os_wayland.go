...
func (w *window) Configure(options []Option) {
	...
	// O Gio de origem cria o zxdg_toplevel_decoration_v1 e só ESCUTA o
	// configure do compositor; nunca manda set_mode. Resultado prático no
	// KWin: app.Decorated(false) não tem efeito nenhum — o compositor fica
	// no padrão server-side e desenha a barra dele POR CIMA da barra que o
	// app desenhou, gastando duas faixas de altura. Aqui o pedido é
	// enviado de verdade.
	// Sem condição de "mudou": na primeira Configure os dois lados já valem
	// false (o zero-value do Config e o Decorated(false) pedido pelo app),
	// e era justamente essa a chamada que precisava sair.
	if w.decor != nil {
		w.config.Decorated = cnf.Decorated
		modo := C.uint32_t(C.ZXDG_TOPLEVEL_DECORATION_V1_MODE_CLIENT_SIDE)
		if cnf.Decorated {
...
func (w *window) updateOpaqueRegion() {
	...
	// Sem decoração do servidor o app desenha a própria moldura, e ela tem
	...
	if C.int32_t(w.size.X) > 2*m && C.int32_t(w.size.Y) > 2*m {
		C.wl_region_add(reg, m, m, C.int32_t(w.size.X)-2*m, C.int32_t(w.size.Y)-2*m)
	} else {
