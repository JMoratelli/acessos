...
typedef struct {
    ...
    /* PROTEGE gdi->primary_buffer (+ width/height/stride) contra a corrida
     * entre um resize (hook_desktop_resize, na thread de rede, que REALOCA
     * o buffer via gdi_resize) e uma cópia em andamento do lado Go — ao
     * contrário do vncshim, o gdi da libfreerdp não guarda o buffer antigo
...
/* Copia tamanho+conteudo do framebuffer atomicamente em relacao a um
 * resize (ver fb_lock). Devolve um buffer malloc'd do tamanho exato
 * (stride*altura) que o chamador deve liberar com rs_liberar_quadro — ou
 * NULL se nao ha framebuffer ainda (ex.: antes do primeiro PostConnect).
 *
 * SUBSTITUI usar rs_framebuffer/rs_largura/rs_altura/rs_stride em
 * sequencia: chamadas separadas nao sao atomicas entre si, e um resize no
